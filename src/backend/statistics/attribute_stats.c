...
attribute_statistics_update(FunctionCallInfo fcinfo, int elevel)
{
	...
	stats_check_arg_array(fcinfo, attarginfo, MOST_COMMON_FREQS_ARG,
						  &do_mcv, elevel);
	stats_check_arg_array(fcinfo, attarginfo, MOST_COMMON_ELEM_FREQS_ARG,
						  &do_mcelem, elevel);
	stats_check_arg_array(fcinfo, attarginfo, ELEM_COUNT_HISTOGRAM_ARG,
						  &do_dechist, elevel);
	...
	stats_check_arg_pair(fcinfo, attarginfo,
						 ...
						 &do_range_length_histogram, elevel);
			   ...
			   Datum stavalues, bool stavalues_isnull)
{
	...
	replaces[Anum_pg_statistic_stakind1 - 1 + slotidx] = true;
	values[Anum_pg_statistic_staop1 - 1 + slotidx] = staop;
	replaces[Anum_pg_statistic_staop1 - 1 + slotidx] = true;
	values[Anum_pg_statistic_stacoll1 - 1 + slotidx] = stacoll;
	replaces[Anum_pg_statistic_stacoll1 - 1 + slotidx] = true;
