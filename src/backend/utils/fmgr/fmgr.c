					   ...
					   bool ignore_security)
{
	...
	if (!ignore_security &&
		(procedureStruct->prosecdef ||
		 (procedureStruct->provolatile == PROVOLATILE_IMMUTABLE &&
...
fmgr_security_definer(PG_FUNCTION_ARGS)
{
	...
	if (!fcinfo->flinfo->fn_extra)
	{
		...
		if (procedureStruct->provolatile == PROVOLATILE_IMMUTABLE)
			fcache->restrict_search_path = true;
	...
	if (fcache->restrict_search_path ||
		fcache->configNames != NIL) /* Need a new GUC nesting level */
