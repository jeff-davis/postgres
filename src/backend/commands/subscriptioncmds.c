				   ...
				   bool isTopLevel)
{
	...
	if (stmt->servername)
	{
		...
		if (server->fdwid != PG_CONNECTION_FDW_OID)
			ereport(ERROR,
					(errcode(ERRCODE_WRONG_OBJECT_TYPE),
					 errmsg("server for subscription must use foreign data wrapper \"pg_connection_fdw\"")));
		...
		case ALTER_SUBSCRIPTION_SERVER:
			{
				...
				if (new_server->fdwid != PG_CONNECTION_FDW_OID)
					ereport(ERROR,
							(errcode(ERRCODE_WRONG_OBJECT_TYPE),
							 errmsg("server for subscription must use foreign data wrapper \"pg_connection_fdw\"")));
