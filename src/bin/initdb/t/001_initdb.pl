...
command_ok(
	[ 'initdb', '--no-sync', '--locale-provider=none', '--locale=POSIX',
	  "$tempdir/data7" ],
	'locale provider none with --locale');

command_ok(
	[ 'initdb', '--no-sync', '--locale-provider=none', '--lc-collate=POSIX',
	  "$tempdir/data8" ],
	'locale provider none with --lc-collate');

command_ok(
	[ 'initdb', '--no-sync', '--locale-provider=none', '--lc-ctype=POSIX',
	  "$tempdir/data9" ],
