...
while (my $line = <$FH>)
{
	...
	if ($status eq 'F' || ($status eq 'C' && defined $special{$code}))
	{
		...
		my @simple_lower = ($simple{$code}{Simple_Lowercase});
		my @simple_title = ($simple{$code}{Simple_Titlecase});
		my @simple_upper = ($simple{$code}{Simple_Uppercase});
		while (scalar @simple_lower < $MAX_CASE_EXPANSION)	{ push @simple_lower, 0x000000; }
		while (scalar @simple_title < $MAX_CASE_EXPANSION)	{ push @simple_title, 0x000000; }
		while (scalar @simple_upper < $MAX_CASE_EXPANSION)	{ push @simple_upper, 0x000000; }
		$special{$code} ||= {
			Lowercase => \@simple_lower,
			Titlecase => \@simple_title,
			Uppercase => \@simple_upper,
			Conditions => '0'
		};
